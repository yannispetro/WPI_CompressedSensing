...
while length(A) < maxVariables && ~stopCond
  ...
  % add variable
  if ~useGram
  % cholinsert() finds the cholesky factorization of X augmented by one
  % more row x, using the chol. fact. R of the original matrix X.
    R = cholinsert(R,X(:,I(cidx)),X(:,A));
  end
  if verbose
    fprintf('%d\t\t%d\t\t%d\n', step, I(cidx), length(A) + 1);
  end
  % We exclude the new element from subsequent consideration
  A = [A I(cidx)];
  I(cidx) = [];
  c(cidx) = [];
